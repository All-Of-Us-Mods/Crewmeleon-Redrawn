...
/// you just chose.
/// </summary>
public sealed class BrushPreset(string name, BrushShape shape, int radius, float opacity, float hardness, BrushMask? mask = null)
{
    ...
    public BrushMask? Mask { get; } = mask;

    public static BrushPreset From(string name, BrushSettings brush) =>
        new(name, brush.Shape, brush.Radius, brush.Opacity, brush.Hardness, brush.Mask?.Clone());
    ...
    public bool Matches(BrushSettings brush) =>
        ReferenceEquals(brush.Mask, Mask)
        && brush.Shape == Shape
        && brush.Radius == Radius
    ...
    public static BrushPreset? Deserialize(string value)
    {
        ...
        var mask = parts.Length > 5 && !string.IsNullOrEmpty(parts[5])
            ? BrushMask.FromBase64(parts[5])
            : null;

        return new BrushPreset(parts[0], (BrushShape) shape, radius, opacity, hardness, mask);
    }
}
